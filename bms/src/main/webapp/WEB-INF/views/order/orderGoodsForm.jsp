...
<script>
	...
	function fn_show_order_detail() {
		...
		p_total_order_goods_price.innerHTML = Math.floor(total_order_goods_price).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',') +"원";
		...
		<tbody align=center>
			...
			<c:forEach var="item" items="${myOrderList }" varStatus="cnt">
				<tr>
					...
					<td>
					  <h2>
					     <a href="${pageContext.request.contextPath}/goods/goodsDetail.do?goodsId=${item.goodsId }&fileName=${item.goodsFileName}">${item.goodsTitle }</A>
					...
					<td><h2><fmt:formatNumber value="${item.goodsSalesPrice *0.9}" type="number" />원</h2></td>
					<td><h2>${deliveryPrice[cnt.count-1] }원</h2></td>
					<td><h2>${1500 *item.orderGoodsQty}원</h2></td>
					<td>
					  <h2><fmt:formatNumber value="${item.goodsSalesPrice* 0.9 * item.orderGoodsQty + deliveryPrice[cnt.count-1]}" type="number" />원</h2><!--  -->
					  <input type="hidden" id="h_each_goods_price"  name="h_each_goods_price" value="${item.goodsSalesPrice * 0.9 * item.orderGoodsQty}" />
					</td>
			</tr>
			
			<c:set var="final_total_order_price" value="${final_total_order_price+ (item.goodsSalesPrice * 0.9 * item.orderGoodsQty) + deliveryPrice[cnt.count-1] }" />
			<c:set var="total_order_price"       value="${total_order_price+ item.goodsSalesPrice * 0.9* item.orderGoodsQty}" />
			<c:set var="total_order_goods_qty"   value="${total_order_goods_qty + item.orderGoodsQty }" />
			<c:set var="total_discount_price"   value="${total_discount_price +  item.goodsSalesPrice*0.1 }" />
			<c:set var="total_delivery_price"   value="${total_delivery_price + deliveryPrice[cnt.count-1] }" />
			</c:forEach>
	...
	<table style="width:80%" class="list_view" style="background: #ccffff">
		<tbody>
			...
			<tr align="center">
				...
				<td>
					<p id="p_totalDelivery"> <fmt:formatNumber value="${total_delivery_price}" type="number" />원</p> 
				...
				<td>
					<p id="p_final_totalPrice">
						<font size="5"><fmt:formatNumber value="${final_total_order_price}" type="number" />원 </font>
...
<div class="clear"></div>		
	<div id="layer" style="visibility:hidden">
		<div id="popup_order_detail">
			...
			<br/> 
			  <div class="detail_table">
			  <font size="4"><strong> &#10004; 최종 주문 사항 &#10004; </strong></font><br><br><br>
			<table>
				<tbody align=left>
				 	<tr>
					  <td width=300px>
